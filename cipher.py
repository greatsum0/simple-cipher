...
class Cipher:
    ...
    def encrypt(self, plain_text):
        return self._encrypt_second_phase(self._encrypt_first_phase(plain_text))

    def decrypt(self, cipher_text):
        return self._decrypt_second_phase(self._decrypt_first_phase(cipher_text))

    def _preprocess_plain_text(self, plain_text):
    ...
    def _encrypt_first_phase(self, plain_text):
        plain_text = self._preprocess_plain_text(plain_text)
    ...
    def _encrypt_second_phase(self, cipher_text):
        ...
        return "".join(np.array(list(cipher_text)).reshape((2,len_column)).T.reshape(1,-1).squeeze().tolist())
        

    def _decrypt_first_phase(self, cipher_text):
        len_column = len(cipher_text)//2
        decipher = "".join(
            (np.array(list(cipher_text)).reshape(len_column,2).T).reshape(1,-1).squeeze())
        return self._preprocess_plain_text(decipher)
